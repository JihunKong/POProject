...
export default function TeamDashboard({ params }: { params: { teamId: string } }) {
  ...
  const createTask = async () => {
    try {
      const response = await axios.post(`/api/teams/${params.teamId}/tasks`, newTask);
  ...
  const updateTaskStatus = async (taskId: string, status: string) => {
    try {
      await axios.patch(`/api/teams/${params.teamId}/tasks/${taskId}`, { status });
  ...
  const getRoleRecommendations = async () => {
    try {
      ...
      const response = await axios.post(`/api/teams/${params.teamId}/recommend-roles`);
          ...
          <div className="flex justify-between items-center py-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                {team?.slogan && (
                  <p className="text-sm text-gray-600 italic">"{team.slogan}"</p>
                )}
              </div>
            </div>
